...
class Action_Login extends \ElementorPro\Modules\Forms\Classes\Action_Base {
	...
	public function run( $record, $ajax_handler )
	{
		...
		switch ( $settings['ap_login_via'] ){
				...
				if (is_wp_error($user)){
					switch($user->get_error_code()){
						...
						case 'invalid_username':
							$ajax_handler->add_error_message(__('You entered wrong credentials. Try again!'));
							break;
						default:
			...
			case 'otp' :
				if( ! AP_IS_GOLD ){
					$ajax_handler->add_error_message(__('Login through OTP is only available in Gold subscription.'));
					return;
				}
			...
			case 'password-otp':
				if( ! AP_IS_GOLD ){
					$ajax_handler->add_error_message(__('Login through Password and OTP is only available in Gold subscription.'));
					return;
				}
