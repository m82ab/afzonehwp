...
class Action_Profile extends \ElementorPro\Modules\Forms\Classes\Action_Base {
    ...
    public function display_edit_user_on_users_table(){
        $option = get_option('actions_pack');
        if( !empty( $url = $option['profile_page_url'] ) ) {
    ...
    public function get_user_field( $user_id, $field){
        switch ($field){
            ...
            case 'name' :
                return trim( get_user_meta($user_id, 'first_name', true)) . ' ' . trim(get_user_meta($user_id, 'last_name', true));
            default :
	...
	public function render_field( $field, $field_index, $form_widget ){
        ...
        foreach ( $settings['ap_profile_fields'] as $profile_field ){
            ...
            if( $profile_field['ap_profile_form_field'] === $field['custom_id'] ){
                ...
                $field_type = 'input';  // This is for text field type. No idea why text field type are considered as input
                $attributes = [];
                ...
                if( $field['field_type'] === 'upload' ){
                    ...
                    foreach ($options as $index=>$option){
                        $form_widget->add_render_attribute( $field['custom_id'] . $index, ['onchange' => 'if(this.hasAttribute("checked")){this.removeAttribute("checked")}else{this.setAttribute("checked", "checked")}']);
                    }
                    ...
                    $options = preg_split( "/\\r\\n|\\r|\\n/", $field['field_options'] );
                    foreach ($options as $index=>$option){
                        $form_widget->add_render_attribute( $field['custom_id'] . $index, ['onchange' => 'Array.from(this.parentElement.parentElement.querySelectorAll("input")).forEach(e => e.removeAttribute("checked"));this.setAttribute("checked", "checked")']);
                    }
                    $form_widget->add_render_attribute( $field['custom_id'] . array_search(trim($default_value), $options), ['checked' => 'checked', 'data-checked' => 'true' ]);
                }
                else{
                ...
                if( $profile_field['ap_profile_is_editable_field'] !== 'yes' ){
                    ...
                    $attributes['style'] = 'cursor:not-allowed;opacity:0.5';
                }

                $form_widget->add_render_attribute( $field_type . $field_index, $attributes);
            }
        }
    ...
    public function register_tags( $dynamic_tags ){
        //TODO: Only register for author archive
        $dynamic_tags->register_tag( \Actions_Pack\Dynamic_Tags\Image_Meta::class );
        $dynamic_tags->register_tag( \Actions_Pack\Dynamic_Tags\Url_Meta::class );
    }
    ...
    public function run( $record, $ajax_handler ) {
        ...
        foreach ( $fields as $item ) {
		    ...
		    if ( (!empty($item['ap_profile_user_field']) || !empty($item['ap_profile_custom_meta']) ) && !empty($item['ap_profile_form_field']) && !empty($form_data[$item['ap_profile_form_field']]) && $item['ap_profile_is_editable_field'] === 'yes'){
